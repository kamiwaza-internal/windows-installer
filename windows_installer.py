...
class KamiwazaInstaller(tk.Tk):
    ...
    def configure_wsl_memory(self):
        ...
        if not is_windows_admin():
            ...
            # Only log, do not show a warning popup
            return False
        try:
    ...
    def get_deb_url(self):
        # Hardcoded URL from config.yaml to avoid config loading issues
        hardcoded_url = "https://pub-3feaeada14ef4a368ea38717abd3cf7e.r2.dev/kamiwaza_v0.5.0_noble_amd64_build4.deb"
        self.log_output(f"Using hardcoded deb URL: {hardcoded_url}")
        return hardcoded_url

    def get_deb_filename(self):
        # Hardcoded filename to match the hardcoded URL
        hardcoded_filename = "kamiwaza_v0.5.0_noble_amd64_build2.deb"
        self.log_output(f"Using hardcoded deb filename: {hardcoded_filename}")
        return hardcoded_filename
    
    def get_wsl_distribution(self):
        """Determine the best WSL distribution to use consistently throughout installation"""
    ...
    def launch_ubuntu_shell(self):
        try:
            # First, try to find available Ubuntu distributions
