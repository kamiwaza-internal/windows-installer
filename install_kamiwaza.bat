@echo off
echo Downloading Kamiwaza Installer...
...
if %ERRORLEVEL%==0 (
    ...
    echo Starting Kamiwaza Installer...
    "%SCRIPT_DIR%kamiwaza_installer_final.exe" --memory=%MEMORY_CONFIG% --version=0.5.0-rc1 --codename=noble --build=1 --arch=amd64 --email="%USER_EMAIL%" --license-key="%LICENSE_KEY%" --usage-reporting=%USAGE_REPORTING% --mode=%INSTALL_MODE%
