...
class KamiwazaInstaller(tk.Tk):
    ...
    def run_wsl_deployment(self, user_wants_kamiwaza):
        ...
        try:
            ...
            # Download the deb file using wget in WSL
            self.update_status("Downloading Kamiwaza package...", 95)
            download_cmd = "wget https://pub-3feaeada14ef4a368ea38717abd3cf7e.r2.dev/kamiwaza_0.4.0-rc1_build01_amd64.deb -P /tmp/"
            result = self.run_command(['wsl', '-d', 'Ubuntu', '-e', 'bash', '-c', download_cmd])
            if result.returncode != 0:
                raise Exception(f"Failed to download Kamiwaza package: {result.stderr}")
            ...
            script = [
                ...
                "sudo dpkg --unpack /tmp/kamiwaza_0.4.0-rc1_build01_amd64.deb",
                "sudo dpkg --configure -a",
                "rm /tmp/kamiwaza_0.4.0-rc1_build01_amd64.deb"  # Clean up the downloaded file
            ]
